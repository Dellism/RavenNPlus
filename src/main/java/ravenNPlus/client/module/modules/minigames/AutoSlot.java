...
public class AutoSlot extends Module {
    ...
    public void p(TickEvent.PlayerTickEvent e) {
        ...
        if(mc.thePlayer.isEntityAlive() && mc.thePlayer.getHealth() > 0 && mc.thePlayer.onGround && !mc.thePlayer.isRiding()
            ...
            if(mode.getValue() == 1) {
                ...
                if (autoDis.isToggled() && mc.thePlayer.getHealth() < autoDisableHealth.getValue() * 2
                        && mc.thePlayer.fallDistance < autoDisDistance.getValue() && mc.thePlayer.experienceLevel == 0) {
                    this.disable();
                }
            }
            ...
            if (mode.getValue() == 3) {
                ...
                if (autoDis.isToggled() && mc.thePlayer.getHealth() < autoDisableHealth.getValue() * 2
                        && mc.thePlayer.fallDistance < autoDisDistance.getValue() && mc.thePlayer.getDisplayNameString().equalsIgnoreCase("0")) {
                    this.disable();
                }
            ...
            if(mode.getValue() == 4) {
                ...
                targets = targets.stream().filter(entity -> entity.getDistanceToEntity(mc.thePlayer) < (int)playerRange.getValue() && entity
                ...
                if (target.getDistanceToEntity(mc.thePlayer) > playerRange.getValue()) {
    ...
    public void onChatMessageRecieved(ClientChatReceivedEvent event) {
        if (mc.thePlayer.isEntityAlive() && mc.thePlayer.getHealth() > 0 && mc.thePlayer.onGround && !mc.thePlayer.isRiding()
            ...
            if (autoDis.isToggled() && mc.thePlayer.getHealth() < autoDisableHealth.getValue() * 2
                    && mc.thePlayer.fallDistance < autoDisDistance.getValue() && (Utils.Java.str(event.message.getUnformattedText()).contains("Start")
