...
import a.b.module.modules.client.CommandPrompt;
import a.c.KeyStroke;
...
public class ClientConfig {
   ...
   public void saveConfig() {
      ...
      config.add(loadedConfigPrefix + Otaku.configManager.getConfig().getName());
      config.add(HUD.HUDX_prefix + HUD.getHudX());
      config.add(HUD.HUDY_prefix + HUD.getHudY());
      config.add(terminalPosPrefix + Otaku.clickGui.terminal.getX() + "," + Otaku.clickGui.terminal.getY());
      config.add(terminalSizePrefix + Otaku.clickGui.terminal.getWidth() + "," + Otaku.clickGui.terminal.getHeight());
      config.add(terminalOpenedPrefix + Otaku.clickGui.terminal.opened);
      config.add(terminalHiddenPrefix + Otaku.clickGui.terminal.hidden);
   ...
   public void applyConfig(){
      ...
      for(String line : config){
         if(line.startsWith(hypixelApiKeyPrefix)){
            ...
            Otaku.configManager.loadConfigByName(line.replace(loadedConfigPrefix, ""));
            ...
            try{
               ...
               Otaku.clickGui.terminal.setLocation(i1, i2);
            } catch (Exception e){}
            ...
            try{
               ...
               int i1 = Integer.parseInt(split_up[0]);
               int i2 = Integer.parseInt(split_up[1]);
               Otaku.clickGui.terminal.setSize(i1, i2);
            } catch (Exception e){}
         } else if(line.startsWith(terminalOpenedPrefix)){
            try{
               Otaku.clickGui.terminal.opened = Boolean.parseBoolean(line.replace(terminalOpenedPrefix, ""));
            } catch (Exception e){}
         } else if(line.startsWith(terminalHiddenPrefix)){
            try{
               CommandPrompt terminalModule = (CommandPrompt) Otaku.moduleManager.getModuleByClazz(CommandPrompt.class);
   ...
   private void loadClickGuiCoords(String decryptedString) {
      for (String what : decryptedString.split("/")){
         for (CategoryComponent cat : Otaku.clickGui.getCategoryList()) {
