...
public class UpdateCheck extends Module {
    ...
    public UpdateCheck() {
        ...
        task = () -> {
            Version latest = Otaku.versionManager.getLatestVersion();
            Version current = Otaku.versionManager.getClientVersion();
            ...
            if (copyToClipboard.isToggled())
                if (Utils.Client.copyToClipboard(Otaku.downloadLocation))
                    Utils.Player.sendMessageToSelf("Successfully copied download link to clipboard!");
                    Utils.Player.sendMessageToSelf(Otaku.sourceLocation);

            if (openLink.isToggled()) {
                try {
                    URL url = new URL(Otaku.sourceLocation);
