...
public class Help extends Command {
    ...
    public void onCall(String[] args) {
        if (args.length == 0) {
            RavenNPlus.commandManager.sort();

            CommandPrompt.print("Available commands:");
            int index = 1;
            for (Command command : RavenNPlus.commandManager.getCommandList()) {
            ...
            Command command = RavenNPlus.commandManager.getCommandByName(args[0]);
            if (command == null) {
