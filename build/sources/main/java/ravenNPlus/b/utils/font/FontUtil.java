package ravenNPlus.b.utils.font;
...
public class FontUtil {
    ...
    public static int getStringWidth(String text) {
        return customFont() ? (RavenNPlus.customFontRenderer.getStringWidth(text) + 3) : fontRenderer.getStringWidth(text);
    }

    public static void drawString(String text, double x, double y, int color) {
        if (customFont()) {
            RavenNPlus.customFontRenderer.drawString(text, x, y - 1, color, false);
        }
        else {
    ...
    public static void drawStringWithShadow(String text, double x, double y, int color) {
        if (customFont()) {
            RavenNPlus.customFontRenderer.drawStringWithShadow(text, x, y - 1, color);
        }
        else {
    ...
    public static void drawCenteredStringWithShadow(String text, float x, float y, int color) {
        if (customFont()) {
            RavenNPlus.customFontRenderer.drawCenteredStringWithShadow(text, x, y - 1, color);
        }
        else {
    ...
    public static void drawCenteredString(String text, float x, float y, int color) {
        if (customFont()) {
            RavenNPlus.customFontRenderer.drawCenteredString(text, x, y - 1, color);
        }
        else {
    ...
    public static int getFontHeight() {
        return customFont() ? (RavenNPlus.customFontRenderer.fontHeight / 2) - 1 : fontRenderer.FONT_HEIGHT;
    }
