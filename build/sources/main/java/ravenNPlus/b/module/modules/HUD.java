package ravenNPlus.b.module.modules;
...
public class HUD extends Module {
   ...
   public HUD() {
      ...
      this.addSetting(editPosition = new TickSetting("Edit position", false));
      this.addSetting(dropShadow = new TickSetting("Drop shadow", true));
      this.addSetting(alphabeticalSort = new TickSetting("Alphabetical sort", false));
      this.addSetting(clientName = new TickSetting("Client Name", true));          //added
      this.addSetting(clientAuthor = new TickSetting("Client Author", false));     //added
   ...
   public void onEnable() { RavenNPlus.moduleManager.sort(); }
   ...
   } else if (b == alphabeticalSort) { RavenNPlus.moduleManager.sort(); } }
   ...
   public void a(RenderTickEvent ev) { if (ev.phase == Phase.END && Utils.Player.isPlayerInGame()) {
       ...
       if(!alphabeticalSort.isToggled()) {
           if(positionMode == Utils.HUD.PositionMode.UPLEFT || positionMode == Utils.HUD.PositionMode.UPRIGHT) {
               RavenNPlus.moduleManager.sortShortLong();
           } else if(positionMode == Utils.HUD.PositionMode.DOWNLEFT || positionMode == Utils.HUD.PositionMode.DOWNRIGHT) {
               RavenNPlus.moduleManager.sortLongShort();
           }
       }

       if(clientName.isToggled())
           mc.fontRendererObj.drawStringWithShadow(RavenNPlus.name, 5, 5, Utils.Client.rainbowDraw(2L, 1500L));
       ...
       List<Module> en = new ArrayList<>(RavenNPlus.moduleManager.getModules());
       if (en.isEmpty()) return;
       int textBoxWidth = RavenNPlus.moduleManager.getLongestActiveModule(mc.fontRendererObj);
       int textBoxHeight = RavenNPlus.moduleManager.getBoxHeight(mc.fontRendererObj, margin);
       if (hudX < 0) {
           hudX = margin;
       }
       if (hudY < 0) {
